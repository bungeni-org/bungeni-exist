...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tocItem" mode="k_part">
        <li><a target="_blank" href="actview.xql?actid={$actid}&amp;pref={$pref}#{$pref}-{substring(@href,2)}"><xsl:value-of select="."></xsl:value-of></a>
            <ul>
            <xsl:apply-templates select="key('k_section',generate-id())" mode="k_section"/>
    ...
    <xsl:template match="tocItem" mode="k_section">
        <li>
            <xsl:if test="position() eq last()">
                <xsl:attribute name="class"><xsl:text>last</xsl:text></xsl:attribute>
            </xsl:if>
            <a  target="_blank" href="actview.xql?actid={$actid}&amp;pref={$pref}#{$pref}-{substring(@href,2)}" ><xsl:value-of select="."></xsl:value-of></a></li>
    </xsl:template>
