        ...
        <eventOf isA="TLCObject">
            <refersTo isA="TLCReference">
                <xsl:attribute name="href">
                    ...
                    <xsl:variable name="event-type" select="//custom/bungeni_doc_type" />
                    <xsl:variable name="event-id" select="//custom/event-id" />
                    
                    <xsl:value-of select="concat($head-item-uri, '/', $event-type, '/', $event-id)" />
                </xsl:attribute>
            </refersTo>  
            <xsl:apply-templates />
        </eventOf>                   
