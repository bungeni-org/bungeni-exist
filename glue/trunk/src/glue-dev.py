...
class ParseBungeniXML:
    ...
    def xpath_contenttype_get_type(self):
        
        root_element = self.xmldoc.getRootElement()
        return self.__global_path__ + "contenttype/field[@name='type']"
    ...
    def get_parliament_info(self):
        ...
        parliament_doc = self.xmldoc.selectSingleNode("contenttype/field[@name='type']")
        ...
        if parliament_doc.getText() == "parliament" :
            ...
            parl_params['parliament-election-date'] = self.xmldoc.selectSingleNode("contenttype/field[@name='election_date']").getText()
            #parl_params['for-parliament'] = "/"+parl_params['country-code']+"/"+self.xmldoc.selectSingleNode("contenttype/field[@name='type']").getText()+"/"+parl_params['parliament-election-date']
            return parl_params
        else:
            return None
...
class ParliamentaryInfoWalker(object):

    def __init__(self, input_params = []):
        """
        dicr_params - the parameters for the callback function
        """
        self.counter = 0
...
def parliament_info(input_file_path, counter, li_params=None):
...
def main(config_file):
    ...
    try:
        ...
        pd = ParliamentaryInfoWalker()
        print "Retrieving parliament information..."
        pd.walk(cfg.get_input_folder(), parliament_info)
        print pd.object_info        
        transformer.set_parl_info(pd.object_info)
