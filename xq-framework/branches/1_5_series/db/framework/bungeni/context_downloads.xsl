...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xqcfg="http://bungeni.org/xquery/config" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:an="http://www.akomantoso.org/1.0" xmlns:nav="http://www.bungeni/org/eXistPortal" xmlns:i18n="http://exist-db.org/xquery/i18n" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:bu="http://portal.bungeni.org/1.0/" exclude-result-prefixes="xs nav" version="2.0">
    ...
    <xsl:template name="doc-formats">
        ...
        <xsl:call-template name="formats-renderer">
            <xsl:with-param name="server-port" select="$server-port"/>
        ...
        <div id="doc-downloads">
            <ul class="ls-downloads">
                <xsl:for-each select="xqcfg:get_downloadgroups($render-group)/format">
                    <xsl:choose>
                        <xsl:when test="@href ne 'null'">
                            <li class="active">
                                <a href="http://{$server-port}/exist/rest/db/framework/bungeni/updater.xql?type={$doc-type}&amp;uri={$uri}" class="{@type}" title="{text()}">
                                    <em>
                        ...
                        <xsl:when test="$uri eq 'null'">
                            <li>
                                <a href="{$doc-type}s/{@type}" class="{@type}" title="{text()}">
                                    <em>
                                        <xsl:value-of select="upper-case(@type)"/>
                                    </em>
                                </a>
                        ...
                        <xsl:otherwise>
                            <li>
                                <a href="{$doc-type}/{@type}?uri={$uri}" class="{@type}" title="{text()}">
                                    <em>
                                        <xsl:value-of select="upper-case(@type)"/>
                                    </em>
                                </a>
