...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:ts="http://www.w3c.org/MarkUp/Forms/XForms/Test/11" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:xf="http://www.w3.org/2002/xforms" version="2.0" exclude-result-prefixes="html ev xsl xf">
    ...
    <xsl:template match="/">
        <html>
            ...
            <body>
                <div class="pageheader">Timetracker Results for Project: <xsl:value-of select="$project"/>
                </div>
                <p/>
                <div>Overall Duration: <xsl:value-of select="/data/project/duration"/>
                </div>
                <table>
                    <tr>
    ...
    <xsl:template match="task">
        <tr>
            ...
            <td>
                <xsl:value-of select="concat(duration/@hours,':',duration/@minutes)"/>
            </td>
            <td>
            ...
            <td>
                <xsl:value-of select="billable"/>
            </td>
            <td>
                <xsl:value-of select="billed/@date"/>
            </td>
        </tr>
    </xsl:template>
</xsl:stylesheet>