                    ...
                    <div id="toggle-wrapper" class="clear toggle-wrapper">
                        ...
                        <xsl:if test="bu:ontology/bu:group/bu:status/bu:value = 'active'">
                            <div class="sub-toggler" id="member-statuses">
                                <span class="list-dot {if($mem-status='current') then 'current' else ()}">•&#160;<a href="{$chamber}/jointcommittee-members?uri={$doc-uri}&amp;status=current">
                                        <i18n:translate>
                                            <i18n:text key="current">current ({1}) (nt)</i18n:text>
                                            <i18n:param>
                                                <xsl:value-of select="count(bu:ontology/bu:members/bu:member[bu:type/bu:value eq 'joint_committee_member' and empty(bu:endDate)])"/>
                                            </i18n:param>
                                        </i18n:translate>
                                    </a>
                                </span>
                                &#160;
                                <span class="list-dot {if($mem-status = 'current') then () else 'current'}">•&#160;<a href="{$chamber}/jointcommittee-members?uri={$doc-uri}&amp;status=past">
                                        <i18n:translate>
                                            <i18n:text key="former">former ({1}) (nt)</i18n:text>
                                            <i18n:param>
                                                <xsl:value-of select="count(bu:ontology/bu:members/bu:member[bu:type/bu:value eq 'joint_committee_member' and not(empty(bu:endDate))])"/>
                                            </i18n:param>
                                        </i18n:translate>
                                    </a>
                                </span>
                    ...
                    <ul id="list-toggle" class="ls-row clear">
                        <xsl:choose>
                            <xsl:when test="$mem-status = 'current'">
                                <xsl:for-each select="bu:ontology/bu:members/bu:member[bu:type/bu:value eq 'joint_committee_member' and empty(bu:endDate)]">
                            ...
                            <xsl:otherwise>
                                <xsl:for-each select="bu:ontology/bu:members/bu:member[bu:type/bu:value eq 'joint_committee_member' and not(empty(bu:endDate))]">
                                    <xsl:sort select="bu:designations/bu:designation/bu:sortOrder" order="ascending" data-type="number"/>
                                    <xsl:call-template name="mem-list-details"/>
                                </xsl:for-each>
                            </xsl:otherwise>
