                        ...
                        <xsl:when test="bu:ontology/bu:members/bu:member">
                            <div id="toggle-wrapper" class="clear toggle-wrapper">
                                ...
                                <div class="sub-toggler" id="member-statuses">
                                    <span class="list-active">•&#160;<a href="{$chamber}/politicalgroup-members?uri={$doc-uri}&amp;status=current">
                                            <i18n:translate>
                                    ...
                                    &#160;
                                    <span class="list-inactive">•&#160;<a href="{$chamber}/politicalgroup-members?uri={$doc-uri}&amp;status=past">
                                            <i18n:translate>
                            ...
                            <ul id="list-toggle" class="ls-row clear">
                                <xsl:choose>
                                    <xsl:when test="$mem-status = 'current'">
                                        <xsl:for-each select="bu:ontology/bu:members/bu:member[bu:type/bu:value eq 'political_group_member' and empty(bu:endDate)]">
                                    ...
                                    <xsl:otherwise>
                                        <xsl:for-each select="bu:ontology/bu:members/bu:member[bu:type/bu:value eq 'political_group_member' and not(empty(bu:endDate))]">
                                            <xsl:sort select="bu:designations/bu:designation/bu:sortOrder" order="ascending" data-type="number"/>
                                            <xsl:call-template name="mem-list-details"/>
                                        </xsl:for-each>
                                    </xsl:otherwise>
